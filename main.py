...
Попробуйте снова позже или используйте другой API."""
    ...
    def _generate_comparison_prompt(individual_analyses_data):
        ...
        prompt += "Твоя задача — на основе этих (невидимых тебе сейчас, но предполагаемых развернутых) индивидуальных анализов, предоставить ТОЛЬКО ИТОГОВЫЕ ОБЩИЕ РЕКОМЕНДАЦИИ И ВЫВОДЫ для пользователя, который пытается выбрать между этими товарами.\n\n"
        prompt += "ВАЖНЫЕ УКАЗАНИЯ:\n"
        prompt += "- Твои рекомендации должны быть сбалансированными. Учитывай суть отзывов, а не только их количество. Небольшое количество качественных, подробных отзывов может быть более информативным, чем множество коротких или противоречивых.\n"
        ...
        prompt += "### 🔥 Выбор редакции (если есть явный лидер на основе КАЧЕСТВЕННОГО анализа содержания отзывов и УВЕРЕННОГО анализа):\n"
        prompt += "[Назови товар, который считаешь лучшим выбором. Обязательно УЧИТЫВАЙ СОДЕРЖАНИЕ И КАЧЕСТВО отзывов, а не только их количество. Если у товара мало отзывов или анализ был неубедительным, он НЕ МОЖЕТ быть выбором редакции. Если явного лидера нет или по всем товарам недостаточно данных, честно укажи это. Объясни свой выбор (3-4 предложения).]\n\n"

        prompt += "### ⚠️ Возможные компромиссы и предостережения:\n"
        prompt += "[Укажи, на какие компромиссы придется пойти при выборе каждого из товаров, или какие у них есть недостатки, важные для определенных групп пользователей. Если для какого-то товара было мало отзывов, отметь это как риск или причину для осторожности, но не делай это единственным критерием.]\n\n"
